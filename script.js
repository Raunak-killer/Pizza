...
document.getElementById('apply-promo')?.addEventListener('click', () => {
  ...
  if (!validCoupons[code]) {
    msg.textContent = "❌ Invalid promo code.";
    updateCartDisplay(); // Reset cart total
    return;
  }

  const discount = validCoupons[code];
  let discountAmount = typeof discount === 'number' && discount < 1
    ? total * discount
    : discount;
...
// Override updateCartDisplay to handle optional discounted total
function updateCartDisplay(finalTotalOverride) {
  ...
  totalElem.textContent = finalTotalOverride || total.toFixed(2);
}
